...
import { ORG_LEFT_LOGO_URL, ORG_REPORT_SUBTITLE, ORG_REPORT_TITLE, ORG_RIGHT_LOGO_URL, WE_LOGO_URL } from "../utils/branding";

export default function BrandHeader({ sectionTitle = "", sectionHint = "", className = "", showWeLogo = true }) {
  return (
    <div className={clsx("rounded-3xl border shadow-sm p-4 md:p-5 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm", className)} dir="rtl">
      {showWeLogo && (
        <div className="flex justify-center mb-4 pb-4 border-b border-blue-200 dark:border-blue-900">
          <img src={WE_LOGO_URL} alt="We Logo" className="h-10 md:h-12 w-auto object-contain" />
        </div>
      )}
