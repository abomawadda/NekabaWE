...
export const renderPrintReport = ({
  ...
  const orientation = config.orientation || (fields.length > 7 ? "landscape" : "portrait");
  const reportMeta = [
    `الفترة: ${period.from || "—"} إلى ${period.to || "—"}`,
    `عدد السجلات: ${rows.length}`,
    `ترتيب الفرز: ${config.sortLabel}`,
  ...
  const totalsRow = summary.currencyFields.some((field) => selectedFields.includes(field.key))
    ? `
      <tr class="totals-row">
      ...
      <head>
        ...
        <style>
          ...
          *{box-sizing:border-box;margin:0;padding:0;font-family:'Cairo',sans-serif}
          body{padding:14px;color:#0f172a;background:#fff;font-size:11px}
          ...
          .report-note{padding:10px 12px;border:1px solid #e2e8f0;border-radius:12px;background:#f8fafc;color:#475569;font-weight:700;font-size:10px}
          table{width:100%;border-collapse:collapse}
          th{background:#0f172a;color:#fff;padding:9px 7px;border:1px solid #334155;font-size:10px;text-align:right}
          td{border:1px solid #cbd5e1;padding:7px;font-size:10px;vertical-align:top}
      ...
      <body>
        <div class="sheet">
          ...
          <div class="report-note">
            ${escapeHtml(config.subtitle)}<br/>
            ترتيب البيانات داخل هذا التقرير: ${escapeHtml(config.sortLabel)}
          </div>
          <table>
            <thead>
              <tr>
